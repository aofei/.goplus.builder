...
func (s *Spxls) HandleMessage(this js.Value, args []js.Value) any {
	if len(args) != 1 {
		return errors.New("HandleMessage: expected 1 argument")
	}
	if args[0].Type() != js.TypeObject {
		return errors.New("HandleMessage: message argument must be an object")
	}
...
func (s *Spxls) ReplyMessage(m jsonrpc2.Message) error {
...
func main() {
	js.Global().Set("Spxls", JSFuncOfWithError(NewSpxls))
	js.Global().Get("eval").Invoke(`
(() => {
	const originalSpxls = globalThis.Spxls;
	globalThis.Spxls = function(...args) {
		const instance = originalSpxls.apply(this, args);
