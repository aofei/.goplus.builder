		...
		case *gopast.DeclStmt:
			if genDecl, ok := node.Decl.(*gopast.GenDecl); ok && (genDecl.Tok == goptoken.VAR || genDecl.Tok == goptoken.CONST) {
				for _, spec := range genDecl.Specs {
					...
					ctx.kind = completionKindAssignment
					ctx.expectedType = ctx.result.typeInfo.TypeOf(valueSpec.Type)
					break
				}
			}
...
func (ctx *completionContext) isInImportStringLit() bool {
	...
	s.Init(ctx.result.fset.File(ctx.astFile.Pos()), ctx.astFile.Code, nil, 0)
...
func (ctx *completionContext) collectCall() error {
	...
	if fun != nil {
		...
		if len(funcOverloads) > 0 {
			...
			for _, expectedType := range slices.Compact(expectedTypes) {
	...
	if argIndex < sig.Params().Len() {
		return ctx.collectTypeSpecific(sig.Params().At(argIndex).Type())
	} else if sig.Variadic() && argIndex >= sig.Params().Len()-1 {
		return ctx.collectTypeSpecific(sig.Params().At(sig.Params().Len() - 1).Type().(*types.Slice).Elem())
	}
	return nil
}
...
// collectAssignment collects assignment completions.
func (ctx *completionContext) collectAssignment() error {
	return ctx.collectTypeSpecific(ctx.expectedType)
}
	...
	case spxSpriteTypeFullName, spxSpriteImplTypeFullName:
		...
		for spxSprite := range ctx.result.spxSpriteResourceAutoBindings {
			if spxSprite.Type().String() == typ.String() {
				spxResourceNames = append(spxResourceNames, spxSprite.Name())
			}
		}
	...
	case spxSoundTypeFullName:
		...
		for spxSound := range ctx.result.spxSoundResourceAutoBindings {
			if spxSound.Type().String() == typ.String() {
				spxResourceNames = append(spxResourceNames, spxSound.Name())
			}
		}
...
func (ctx *completionContext) collectSwitchCase() error {
	if ctx.switchTag == nil {
		for _, typ := range []string{"int", "string", "bool", "error"} {
			ctx.itemSet.addSpxDefs(GetSpxBuiltinDefinition(typ))
		}
		return nil
	}
