...
// Markdown returns the Markdown representation of the definition.
func (def SpxDefinition) Markdown() string {
...
func (def SpxDefinition) CompletionItem() CompletionItem {
	return CompletionItem{
		...
		Documentation:    &Or_CompletionItem_documentation{Value: MarkupContent{Kind: Markdown, Value: def.Markdown()}},
...
// NewSpxDefinitionForPkgName creates a new [SpxDefinition] for the provided
// package name.
func NewSpxDefinitionForPkgName(result *compileResult, pkgName *types.PkgName) SpxDefinition {
	pkg := pkgName.Pkg()
	pkgPath := pkg.Path()
